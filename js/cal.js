...
function cal() {
    ...
    passenger.pType= document.getElementById("passenger_type").selectedIndex-1;
    passenger.vip= document.getElementById("vip_type").selectedIndex-1;
    passenger.cabin= document.getElementById("cabin_type").selectedIndex-1;
    passenger.airLine= document.getElementById("air_line").selectedIndex-1;
    passenger.tPrice= parseFloat(document.getElementById("ticket_price").value);
    ...
    for(var i = 1; i<rows.length; i++ ){
        ...
        baggage.special = parseInt(rows[i].cells[5].innerHTML);
        baggage.size = parseFloat(rows[i].cells[2].innerHTML);
        baggage.weight = parseFloat(rows[i].cells[3].innerHTML);
        baggages[num++] = baggage;
    }
    var cost = calculate(passenger, baggages);
...
function addTmplFile() {
    ...
    // 将input中的值变为空
    document.getElementById("baggage_type").options[0].selected = true;
    document.getElementById("baggage_size_l").value = null;
    document.getElementById("baggage_size_w").value = null;
    document.getElementById("baggage_size_h").value = null;
    document.getElementById("baggage_weight").value = null;
}

// 提交1
function submit1() {
    // 获取输入的值（即input中的value值）
    var bt_sel = document.getElementById("baggage_type");
    var bt_sel_index = bt_sel.selectedIndex;//baggage_type_selected_index
    var bt = bt_sel.options[bt_sel_index].text;
    var b_size_l = parseFloat(document.getElementById("baggage_size_l").value);
    var b_size_w = parseFloat(document.getElementById("baggage_size_w").value);
    var b_size_h = parseFloat(document.getElementById("baggage_size_h").value);
    var b_size = b_size_l + b_size_w + b_size_h;
    var b_weight = parseFloat(document.getElementById("baggage_weight").value);

    if(isEmpty(b_size_l)||isEmpty(b_size_w)||isEmpty(b_size_h)||isEmpty(b_weight)){
        alert("请输入完整信息！");
        return;
    }
    ...
    //添加一行
    var tradd = tab.insertRow(rows)
    tradd.insertCell(0).innerText=rows;
    //tradd.insertCell(1).innerText=bt_sel_index;
    tradd.insertCell(1).innerText=bt;
    tradd.insertCell(2).innerText=b_size;
    tradd.insertCell(3).innerText=b_weight;

    // 改变原本得到的数据格式为textNode格式
    bt_sel_index = document.createTextNode(bt_sel_index);
/*    b_size = document.createTextNode(b_size);
    b_weight = document.createTextNode(b_weight);*/
...
/*    // 创建tr（创建行）
    ...
    td1.className = "col1";
    td1.appendChild(rows);*/

    var td0 = document.createElement("td");
    td0.className = "col0";
    td0.appendChild(bt_sel_index);
